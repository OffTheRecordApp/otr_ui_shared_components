...
class MultiSelectDropdown {
    ...
    public onOptionSelection(option) {
        const isOptionSelected = findIndex(this.selectedOptions, (o) => o === option);
      ...
      */
        ...
        if (option.name === this.overrideOption?.name) {
            ...
            this.overrideOption &&
            this.selectedOptions.includes(this.overrideOption) &&
            option !== this.overrideOption
        ) {
            remove(this.selectedOptions, (o) => o === this.overrideOption);
        }

        // if findIndex returns -1 option is not in list
        if (isOptionSelected > -1) {
            remove(this.selectedOptions, (o) => o === option);
